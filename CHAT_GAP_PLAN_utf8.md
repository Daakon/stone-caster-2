# Chat E2E (Non-Streaming JSON) ΓÇö Gap & Implementation Plan
...
```
Client ΓåÆ POST /api/chat/turn
  { gameId, input, idempotencyKey }

Server:
  1. Assemble full prompt (core ΓåÆ ruleset ΓåÆ world ΓåÆ entry ΓåÆ entry_start (turn 1) ΓåÆ npc ΓåÆ game_state ΓåÆ player ΓåÆ rng ΓåÆ input)
...
Client: Displays "processing..." ΓåÆ receives full turn ΓåÆ renders
```
...
1. Create strict JSON validator at adapter layer:
   ...
   - Behavior: Parse ΓåÆ validate ΓåÆ throw if invalid ΓåÆ return typed object
2. Wire auto-repair:
   - On JSON.parse failure ΓåÆ call `OpenAIService.repairJSONResponse`
   - Retry once ΓåÆ throw if repair fails
...
- [ ] Unit test: malformed JSON ΓåÆ auto-repair ΓåÆ success
- [ ] Unit test: irreparable JSON ΓåÆ throw TurnValidationError
...
- [ ] Unit test: valid JSON ΓåÆ returns object
- [ ] Unit test: invalid JSON ΓåÆ throws

---

### Gap 3: Turn Handler Flow (Apply Deltas Atomically)

**Why It Matters**: Current flow: player turn insert ΓåÆ model call ΓåÆ narrator turn insert ΓåÆ delta application. No transaction wrapper. Partial failure leaves inconsistent state.
...
2. Create stored procedure `turn_atomic_apply`:
   ...
   - Steps: insert player turn ΓåÆ insert narrator turn ΓåÆ update game state ΓåÆ update relationships ΓåÆ return narrator turn
...
- [ ] Unit test: model failure ΓåÆ no player turn persisted
- [ ] Unit test: delta failure ΓåÆ rollback narrator turn
- [ ] Integration test: full turn ΓåÆ all state updated atomically
...
1. **Input Moderation** (pre-input check):
   ...
   - Behavior: POST { input: text } ΓåÆ check flagged ΓåÆ reject if true
...
- [ ] Unit test: flagged input ΓåÆ 400 MODERATION_REJECTED
- [ ] Unit test: over quota ΓåÆ 402 INSUFFICIENT_QUOTA
...
3. Wire into assembler:
   ...
   - Check cache ΓåÆ hit: load prefix + append dynamic ΓåÆ miss: assemble full + cache prefix
...
- [ ] Unit test: first turn ΓåÆ miss ΓåÆ cache populated
- [ ] Unit test: second turn ΓåÆ hit ΓåÆ faster assembly
...
- [ ] Unit test: user inputs `=== CORE_BEGIN ===` ΓåÆ stripped
...
- [ ] **12. Integration Test**: Full turn flow (input ΓåÆ JSON ΓåÆ deltas ΓåÆ response) end-to-end
- [ ] **13. Load Test**: 10 concurrent turns on same game ΓåÆ all succeed or fail gracefully
...
1. **Gap 2** (Adapter JSON method) ΓÇö Foundation (1-2 hrs)
2. **Gap 1** (JSON validation) ΓÇö Build on #1 (3-5 hrs)
3. **Gap 5** (Token accounting) ΓÇö Independent, quick win (1-2 hrs)
4. **Gap 3** (Atomic transaction) ΓÇö Core correctness (4-6 hrs)
5. **Gap 6** (UI processing UX) ΓÇö User-facing polish (1 hr)
6. **Risk 2** (Timeout) ΓÇö Safety (1 hr)
7. **Risk 4** (Prompt injection) ΓÇö Safety (1 hr)
8. **Gap 4** (Moderation & caps) ΓÇö Post-MVP (6-8 hrs)
9. **Gap 7** (Telemetry) ΓÇö Post-MVP (3-4 hrs)
10. **Gap 8** (Cache) ΓÇö Post-MVP (4-5 hrs)
