...
// Note: Story IDs are now generated by the database via gen_random_uuid() default
// No need to generate IDs in code - let the database handle it
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Only include description_short if it's provided (defensive against missing column)
      ...
      // Add description_short if provided (may fail if column is missing - handled in error)
      if (storyData.description_short !== undefined && storyData.description_short !== null) {
        storyInsertData.description_short = storyData.description_short;
      }
      ...
      if (storyError) {
        ...
        if (storyError.code === 'PGRST204') {
          const columnName = storyError.message?.match(/column '(\w+)'/)?.[1] || 'unknown';
          return sendErrorWithStatus(
            res,
            ApiErrorCode.INTERNAL_ERROR,
            `Database schema cache error: The '${columnName}' column is not recognized. Please refresh your Supabase schema cache (e.g., via SQL Editor: SELECT pg_notify('pgrst', 'reload schema');). Original message: ${storyError.message}`,
            req
          );
        }
      ...
      if (!story || !story.id) {
        return sendErrorWithStatus(
          res,
          ApiErrorCode.INTERNAL_ERROR,
          'Failed to create story: no ID returned',
          req
        );
      }

      // Get the UUID ID from the inserted story (database-generated)
      ...
      if (storyData.ruleset_template_ids && storyData.ruleset_template_ids.length > 0) {
        ...
        if (linksError) {
          ...
          await supabaseAdmin.from('chimera_stories').delete().eq('id', storyId);
          return sendErrorWithStatus(
            res,
            ApiErrorCode.INTERNAL_ERROR,
      ...
      if (storyData.pack_ids && storyData.pack_ids.length > 0) {
        ...
        if (packLinksError) {
          ...
          await supabaseAdmin.from('chimera_story_links').delete().eq('story_id', storyId);
          await supabaseAdmin.from('chimera_stories').delete().eq('id', storyId);
          return sendErrorWithStatus(
            res,
            ApiErrorCode.INTERNAL_ERROR,
      ...
      if (storyData.entity_ids && storyData.entity_ids.length > 0) {
        ...
        if (entityLinksError) {
          ...
          await supabaseAdmin.from('chimera_story_content_pack_links').delete().eq('story_id', storyId);
          await supabaseAdmin.from('chimera_story_links').delete().eq('story_id', storyId);
          await supabaseAdmin.from('chimera_stories').delete().eq('id', storyId);
          return sendErrorWithStatus(
            res,
