-- Naming & Reference Semantics Cleanup Migration (Safe Version)
...
CREATE POLICY epr_owner_select ON public.entry_point_rulesets FOR SELECT USING (EXISTS (SELECT 1 FROM public.entry_points ep WHERE ep.id = entry_point_id AND (ep.owner_user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.app_roles r WHERE r.user_id=auth.uid() AND r.role IN ('moderator','admin'))));

-- Policy for INSERT operations
CREATE POLICY epr_owner_insert ON public.entry_point_rulesets FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.entry_points ep WHERE ep.id = entry_point_id AND (ep.owner_user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.app_roles r WHERE r.user_id=auth.uid() AND r.role IN ('moderator','admin'))));

-- Policy for UPDATE operations
CREATE POLICY epr_owner_update ON public.entry_point_rulesets FOR UPDATE USING (EXISTS (SELECT 1 FROM public.entry_points ep WHERE ep.id = entry_point_id AND (ep.owner_user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.app_roles r WHERE r.user_id=auth.uid() AND r.role IN ('moderator','admin')))) WITH CHECK (EXISTS (SELECT 1 FROM public.entry_points ep WHERE ep.id = entry_point_id AND (ep.owner_user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.app_roles r WHERE r.user_id=auth.uid() AND r.role IN ('moderator','admin'))));

-- Policy for DELETE operations
CREATE POLICY epr_owner_delete ON public.entry_point_rulesets FOR DELETE USING (EXISTS (SELECT 1 FROM public.entry_points ep WHERE ep.id = entry_point_id AND (ep.owner_user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.app_roles r WHERE r.user_id=auth.uid() AND r.role IN ('moderator','admin'))));
