...
/**
 * Filesystem prompt assembler that loads markdown/JSON templates and
 * assembles them into a complete prompt for the AI runtime.
 */
export class PromptAssembler {
  private manifest: Awaited<ReturnType<typeof getPromptManifest>> | null;
  ...
  /**
   * Initialize the manifest cache if needed.
   */
  ...
  /**
   * Assemble a prompt using the filesystem manifest.
   */
  async assemblePrompt(context: PromptContext): Promise<PromptAssemblyResult> {
    ...
    if (!this.manifest) {
      throw new Error('Prompt manifest failed to initialize');
    }
    ...
    if (!validation.valid) {
      console.warn('[PROMPT_ASSEMBLER] Missing required templates:', validation.missing);
    }
    ...
    for (const template of templates) {
      try {
        const processed = await this.processTemplate(template, variableContext);
        segments.push(...processed);
        templateIds.push(template.id);
      } catch (error) {
        const warning = `Failed to process template ${template.id}: ${error instanceof Error ? error.message : String(error)}`;
        warnings.push(warning);
        console.warn(`[PROMPT_ASSEMBLER] ${warning}`);
      }
    }

    const prompt = this.createFinalPrompt(segments, context);
    const audit = await this.buildAuditEntry(templateIds, templates, context, prompt);
  ...
  private async processTemplate(
    ...
    for (const segment of template.segments) {
      ...
      if (!validation.valid) {
        console.warn(
          `[PROMPT_ASSEMBLER] Template ${template.id} contains invalid variables: ${validation.invalidVariables.join(', ')}`
        );
      }

      let processed = replaceTemplateVariables(segment, contextMap);
  ...
  private async processFileInclusions(segment: string): Promise<string> {
    ...
    while ((match = filePattern.exec(segment)) !== null) {
      ...
      try {
        ...
        console.log(`[PROMPT_ASSEMBLER] Included file: ${relativePath} (${content.length} chars)`);
      } catch (error) {
        console.warn(`[PROMPT_ASSEMBLER] Could not include file ${relativePath}:`, error);
      }
    }
  ...
  private buildContextObject(context: PromptContext): Record<string, unknown> {
    ...
    return {
      ...
      // Legacy helpers used by templates
  ...
  private async buildAuditEntry(
    ...
    if (!this.manifest) {
      throw new Error('Prompt manifest not initialized');
    }
  ...
  private getPromptVersion(templates: PromptTemplateMeta[]): string {
    const versions = templates.map((template) => template.version).filter(Boolean);
  ...
  async getWorldConfig(worldId: string): Promise<unknown> {
    if (!this.manifest) {
      await this.initialize();
    }
