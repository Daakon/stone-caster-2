...
```
┌───────────────────────────────────────────────────────────────────────┐
│ Logo        Worlds  NPCs  Stories                 Play        Create   │
│                                                                     ◉ │
└───────────────────────────────────────────────────────────────────────┘
                     ↑                      ↑                   ↑
...
- StoneCaster logo and **Play** both land on `/play` for quick resumption of sessions.
- **Worlds** and **NPCs** open discovery browsers, which can chain into modals showing worlds, entities, and stories.
- **Create** (My Creations) routes to dashboard tabs; Stories tab links straight into Casting Circle via **Cast New Story**.
...
6. **Session Controls:** Return to Play hub, Abandon, Download transcript (for authors), and bug report hook.
...
## 3.2 Start Story & Character Selection

```
Story Summary (cover, world, difficulty, stone cost)

Characters Tab:
[ + Create New ]  [Quick Start]  [Filters v]
┌───────────────┐ ┌───────────────┐ ┌───────────────┐
│ Existing Char │ │ Existing Char │ │ Quick Start   │
│ Name          │ │ Name          │ │ Template      │
│ World badges  │ │ Last Played   │ │ Role icon     │
│ [Select ▶]    │ │ [Select ▶]    │ │ [Duplicate ▶] │
└───────────────┘ └───────────────┘ └───────────────┘

Character Drawer (on select):
Name, pronouns, archetype, stats preview, bio snippet, session usage list.

Footer:
[Back]      [Use Selected Character ▶]    [Create Advanced Character]
```

```mermaid
flowchart LR
    StorySummary --> CharacterLibrary
    CharacterLibrary --> ExistingChar["Existing Character Card"]
    CharacterLibrary --> QuickStart["Quick Start Template"]
    CharacterLibrary --> CreateNew["Create New Character"]
    ExistingChar --> Drawer["Character Drawer"]
    QuickStart -->|Duplicate| Drawer
    CreateNew --> LightCreate["/character/new/:storyId"] --> LibraryUpdate["Save to Library"]
    CreateNew --> AdvancedCreate["/character/create/:storyId"] --> LibraryUpdate
    LibraryUpdate --> Attach["Attach to Session"]
    Drawer --> Attach
    Attach --> SessionStart["POST /api/sessions"]
```

---

## 4. Authoring Dashboard (`/create`)
...
- Quick links to `Play`, `Create`, Purchases history.
