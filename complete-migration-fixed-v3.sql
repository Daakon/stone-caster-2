-- ============================================================================
-- COMPLETE ADMIN MIGRATION (ALL 9 MIGRATIONS COMBINED - FIXED V3)
-- ============================================================================
-- This version handles the actual worlds table schema which doesn't have a status column
-- and safely adds all required columns before referencing them
...
DO $$
BEGIN
  -- Only create the view if the status column exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'worlds' 
    AND table_schema = 'public' 
    ...
    -- Check if description column exists too
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'worlds' 
      AND table_schema = 'public' 
    ...
    ELSE
      -- Create view without description column
      CREATE OR REPLACE VIEW public.worlds_admin AS
      SELECT 
        wm.uuid_id as id,
        w.name,
...
-- Insert some default rulesets with explicit slugs (only if they don't exist)
DO $$
BEGIN
...
BEGIN
  ...
  -- Update existing records to have status (only if column exists)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'entry_points' 
    AND table_schema = 'public' 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'entry_rulesets' 
    AND policyname = 'Entry rulesets: Authenticated users can manage'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'npc_packs' 
    AND policyname = 'NPC Packs: Anyone can view active packs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'npc_pack_members' 
    AND policyname = 'NPC Pack Members: Authenticated users can manage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'entry_npcs' 
    AND policyname = 'Entry NPCs: Authenticated users can manage'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'entry_npc_packs' 
    AND policyname = 'Entry NPC Packs: Authenticated users can manage'
  ) THEN
...
CREATE TABLE IF NOT EXISTS public.ruleset_revisions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ruleset_id text NOT NULL REFERENCES public.rulesets(id) ON DELETE CASCADE,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'ruleset_revisions' 
    AND policyname = 'Ruleset revisions: Admin can read all'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'ruleset_revisions' 
    AND policyname = 'Ruleset revisions: Admin can insert'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'content_import_jobs' 
    AND policyname = 'Import jobs: Admin can do everything'
  ) THEN
...
CREATE OR REPLACE FUNCTION public.publish_ruleset(ruleset_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Get the ruleset
  ...
  INSERT INTO public.rulesets (
    id, name, slug, description, status, version_major, version_minor, version_patch
  ) VALUES (
    ruleset_record.id || '-v' || new_version_major || '-' || new_version_minor || '-' || new_version_patch,
...
-- Insert some sample rulesets with versioning (only if they don't exist)
DO $$
BEGIN
  -- Insert D&D 5e Core if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM public.rulesets WHERE slug = 'd-d-5e-core' AND version_major = 1 AND version_minor = 0 AND version_patch = 0) THEN
    INSERT INTO public.rulesets (id, name, slug, description, status, version_major, version_minor, version_patch) VALUES
      ('d-d-5e-core', 'D&D 5e Core', 'd-d-5e-core', 'Core D&D 5th Edition rules', 'active', 1, 0, 0);
  END IF;

  -- Insert Pathfinder Core if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM public.rulesets WHERE slug = 'pathfinder-core' AND version_major = 2 AND version_minor = 1 AND version_patch = 0) THEN
    INSERT INTO public.rulesets (id, name, slug, description, status, version_major, version_minor, version_patch) VALUES
      ('pathfinder-core', 'Pathfinder Core', 'pathfinder-core', 'Core Pathfinder rules', 'draft', 2, 1, 0);
  END IF;

  -- Insert Custom Fantasy if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM public.rulesets WHERE slug = 'custom-fantasy' AND version_major = 1 AND version_minor = 2 AND version_patch = 3) THEN
    INSERT INTO public.rulesets (id, name, slug, description, status, version_major, version_minor, version_patch) VALUES
      ('custom-fantasy', 'Custom Fantasy', 'custom-fantasy', 'Custom fantasy ruleset', 'active', 1, 2, 3);
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
    ...
    ALTER TABLE public.prompt_segments
      ADD COLUMN ref_world_id text GENERATED ALWAYS AS (CASE WHEN scope='world' THEN ref_id ELSE NULL END) STORED;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema='public' AND table_name='prompt_segments' AND column_name='ref_ruleset_id') THEN
    ALTER TABLE public.prompt_segments
      ADD COLUMN ref_ruleset_id text GENERATED ALWAYS AS (CASE WHEN scope='ruleset' THEN ref_id ELSE NULL END) STORED;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema='public' AND table_name='prompt_segments' AND column_name='ref_entry_id') THEN
    ALTER TABLE public.prompt_segments
      ADD COLUMN ref_entry_id text GENERATED ALWAYS AS (CASE WHEN scope IN ('entry','entry_start') THEN ref_id ELSE NULL END) STORED;
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='fk_ps_entry_ref') THEN
    ALTER TABLE public.prompt_segments
      ADD CONSTRAINT fk_ps_entry_ref FOREIGN KEY (ref_entry_id) REFERENCES public.entry_points(id) NOT VALID;
  END IF;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can view their own NPCs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can create their own NPCs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can update their own NPCs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can delete their own NPCs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Admin can do everything'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can view public NPCs and their own private NPCs'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'npcs' 
    AND policyname = 'NPCs: Users can create NPCs'
  ) THEN
